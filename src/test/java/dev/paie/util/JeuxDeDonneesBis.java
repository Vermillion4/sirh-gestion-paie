...
public class JeuxDeDonneesBis {
	...
	public void test_primeExceptionnelle() {	
		assertThat(bulletin1.getPrimeExceptionnelle()).isEqualByComparingTo(new BigDecimal("1000"));
	}
	...
	public void test_cotisationsNonImposables() {
		...
		.forEach(code -> assertThat(
				cotisationsNonImposables.stream().filter(c -> c.getCode().equals
	...
	public void test_grade() {
		assertThat(bulletin1.getRemunerationEmploye().getGrade().getNbHeuresBase()).isEqualByComparingTo(new BigDecimal("151.67"));
		assertThat(bulletin1.getRemunerationEmploye().getGrade().getTauxBase()).isEqualByComparingTo(new BigDecimal("11.0984"));
